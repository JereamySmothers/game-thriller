...
export const App = () => {
  ...
  return (
    <motion.div
      onAnimationEnd={() => {
        setClicked(false);
        setCharLocation({ top: destination.y, left: destination.x });
      }}
      animate={clicked ? "move" : "still"}
      variants={variants}
      transition={{ duration: 3 }}
      onClick={(e) => handleCharMove(e.pageY, e.pageX)}
      style={{ height: "2000px", width: "2000px" }}
    >
      <InteractObject
        image={tv}
        glowyImage={hdtv}
        location={{ top: "500px", left: "500px" }}
      />
  ...
  return (
    <div>
      <img
        ...
        alt="interact"
        src={isHovered ? glowyImage : image}
        style={{
          border: isHovered ? "2px solid gold" : "",
          height: "50px",
          width: "50px",
          position: "absolute",
          top: location.top,
